...
void ReorderBuffer::Tick() {
  ...
  wire_in.rob_set_pc_enable = false;
  ...
  if (wire_out.rob_append_enable) {
    ...
    auto &entry = entries_[tail_]; // NOLINT
...
ReplyReorderBuffer ReorderBuffer::Query(uint32_t reg_id) const {
  ...
  if (reg_id == tail_ && wire_out.rob_append_enable) {
  ...
  if (wire_out.writeback1_enable && wire_out.writeback1_id == reg_id) {
    ret.ready = true;
    ret.val = wire_out.writeback1_val;
  }
  if (wire_out.writeback2_enable && wire_out.writeback2_id == reg_id) {
    ret.ready = true;
    ret.val = wire_out.writeback2_val;
  }
  return ret;
}
